...
def _check_rec(lang, basedir, rootdir, curdir, autofix):
    ...
    if filename in filenames:
        ...
        filerel = os.path.relpath(filepath, basedir)
        if not os.path.isfile(filepath) or os.path.islink(filepath):
            yield datacheck.Error(u'Page template /{} is not a regular file', filerel)
        else:
            try:
                with open(filepath) as f:
                    template = f.read()
            except IOError as e:
                yield datacheck.Error(u'Page template /{} read error: {}', filerel, e)
            else:
                try:
                    compiled = Template(template)
                except TemplateSyntaxError as e:
                    yield datacheck.Error(u'Page template /{} parse error: {}', filerel, e)
        ...
        if pages.slug_regex.match(filename):
            ...
            if os.path.islink(filepath):
                ...
                if not target.startswith(rootdir + os.sep) and target != rootdir:
                    yield datacheck.Error(u'Redirect /{} goes outside root dir {}', filerel, link)
                elif not os.path.isdir(target):
                    yield datacheck.Error(
                            u'Redirect /{} points to {} which expands to {} which is not a directory',
                            filerel, link, target_path)
                elif not pages.path_regex.match(target_path):
                    yield datacheck.Error(
                            u'Redirect /{} points to {} which expands to {} which is not a valid path',
                            filerel, link, target_path)
                elif link != target_link:
                    yield datacheck.Warning(
                            u'Redirect /{} points to {} but its canonical form is {}',
                ...
                yield datacheck.Error(u'Subpage /{} is not a directory or a symlink', filerel)
    ...
    if curdir == rootdir:
        ...
        if not os.path.islink(rootlink) or os.readlink(rootlink) != u'.':
            yield datacheck.Warning(u'Invalid or missing rootlink /{}/@', lang, autofixable=True)
            if autofix:
    ...
    if filenames:
        ...
        yield datacheck.Warning(u'Unexpected files in /{}: {}', dirrel, u', '.join(filenames))
